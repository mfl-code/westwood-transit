...
The engine fetches TransLink's ~30MB static GTFS bundle. Using **JSZip**, it extracts and parses CSV files (`stop_times.txt`, `trips.txt`, `calendar.txt`) entirely in-memory. This allows for complex relational queries—such as calculating catchable buses based on SkyTrain travel offsets—without a backend database.
